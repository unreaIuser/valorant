...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    bool ret = Begin(name, NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_ChildWindow);
...
static bool BeginNiggerChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* parent_window = g.CurrentWindow;
    ...
    if (!border)
        g.Style.ChildBorderSize = 0.0f;
    bool ret = ImGui::BeginNigger(name, NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_ChildWindow);
    g.Style.ChildBorderSize = backup_border_size;
...
bool ImGui::BeginNiggerChild(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    return BeginNiggerChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags);
}
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoBackground))
            {
                ...
                ImVec2 from = window->Pos + ImVec2(0, window->TitleBarHeight());
                ImVec2 to = window->Pos + window->Size;

                //outer background
                if (!(flags & ImGuiWindowFlags_ChildWindow))
                    window->DrawList->AddRectFilled(ImVec2(from.x - 5, from.y - 5), ImVec2(to.x + 5, to.y + 5), ImGui::GetColorU32({ 0.12f, 0.12f, 0.12f, 1.f }), 0.f, 0);

                //main background
                window->DrawList->AddRectFilled(from, to, ImGui::GetColorU32({ 0.12f, 0.12f, 0.12f, 1.f }), 0.f, 0);

                if (!(flags & ImGuiWindowFlags_ChildWindow)) {
                    //absolute outline
                    window->DrawList->AddRect(ImVec2(from.x - 5, from.y - 5), ImVec2(to.x + 5, to.y + 5), ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 0.f, 0, 1.f);

                    //inner outline
                    window->DrawList->AddRect(from, to, ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 0.f, 0, 1.f);

                    //colored stripe on top of menu (1/2)
                    ImU32 clr1 = ImGui::GetColorU32({ 0.53f, 0.38f, 1.f, 1.f }); //left
                    ImU32 clr2 = ImGui::GetColorU32({ 0.3f, 0.2f, 1.f, 1.f }); //right
                    window->DrawList->AddRectFilledMultiColor(ImVec2(from.x + 5, from.y + 5), ImVec2((to.x - 5) / 2, from.y + 7), clr1, clr2, clr2, clr1);

                    //colored stripe on top of menu (2/2)
                    ImU32 clr3 = ImGui::GetColorU32({ 0.3f, 0.2f, 1.f, 1.f }); //left
                    ImU32 clr4 = ImGui::GetColorU32({ 0.3f, 1.f, 1.f, 1.f }); //right
                    window->DrawList->AddRectFilledMultiColor(ImVec2((to.x - 5) / 2, from.y + 5), ImVec2(to.x - 5, from.y + 7), clr3, clr4, clr4, clr3);
                }
...
bool ImGui::BeginNigger(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    IM_ASSERT(name != NULL);                        // Window name required
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoBackground))
            {
                ...
                window->DrawList->AddText(ImVec2(from.x + 10, from.y - (text_size.y / 2)), ImGui::GetColorU32({ 1.f, 1.f, 1.f, 1.f }), name);

                //links oben nach links unten
                window->DrawList->AddLine(ImVec2(from.x, from.y), ImVec2(from.x, to.y), ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 1.f);

                //links oben zu text
                window->DrawList->AddLine(ImVec2(from.x, from.y), ImVec2(from.x + 7, from.y), ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 1.f);

                //text (rechts) zu rechts oben
                window->DrawList->AddLine(ImVec2(from.x + text_size.x + 14, from.y), ImVec2(to.x, from.y), ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 1.f);

                //rechts oben nach rechts unten
                window->DrawList->AddLine(ImVec2(to.x, from.y), ImVec2(to.x, to.y), ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 1.f);

                //links unten nach rechts unten
                window->DrawList->AddLine(ImVec2(from.x, to.y), ImVec2(to.x + 1, to.y), ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 1.f);
            }
            g.NextWindowData.BgAlphaCond = 0;
