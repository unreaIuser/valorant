...
#include <iostream>
int old_t_1 = 0;
bool CoolButtonEx(bool active, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    {
        float t_anim = ImSaturate(g.LastActiveIdTimer / 0.5f); //0.3f
        t = (1.0f - t_anim);
        old_t_1 = t;
        if (t >= 1 || t <= 0) {
            old_t_1 = 0;
    ...
    //button background
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImGui::GetColorU32({ 0.1f, 0.1f, 0.1f, 1.f }), 0.f, 0);
    ...
    if (active) {
        //0.53f, 0.38f, 1.f, 1.f
        ImU32 clr3 = ImGui::GetColorU32({ 0.53f / 4, 0.38f / 4, 1.f / 4, 1.f }); //left
        ImU32 clr4 = ImGui::GetColorU32({ 0.1f, 0.1f, 0.1f, 1.f }); //right
        window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + 70 - t, bb.Max.y), clr3, clr4, clr4, clr3);
        window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x, bb.Max.y), ImGui::GetColorU32({ 0.53f, 0.38f, 1.f, 1.f }), 2.f);
    }

    //button outline
    window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 0.f, 0, 1.f);
    
    
    ImGui::RenderTextClipped(ImVec2(bb.Min.x + 10, bb.Min.y), ImVec2(bb.Min.x + 10, bb.Max.y), label, NULL, &label_size, style.ButtonTextAlign, &bb); //text
    /*const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);

    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding); //frame
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb); //text
    */
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    std::cout << "\nt: " << t;
    //button background
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImGui::GetColorU32({ 0.13f, 0.13f, 0.13f, 1.f }), 0.f, 0);

    //button outline
    window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 0.f, 0, 1.f);

    //button effect
    ImU32 clr3 = ImGui::GetColorU32({ 0.5f, 0.f, 1.f, 0.2f }); //left
    ImU32 clr4 = ImGui::GetColorU32({ 0.f, 0.f, 0.f, 0.f }); //right
    window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + 60 - t, bb.Max.y), clr3, clr4, clr4, clr3);
...
bool ImGui::SelectionButton(bool active, const char* label, const ImVec2& size_arg)
{
    return CoolButtonEx(active, label, size_arg, 0);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    //background
    ImU32 clr3 = ImGui::GetColorU32({ 0.18f, 0.18f, 0.18f, 1.0f }); //top
    ImU32 clr4 = ImGui::GetColorU32({ 0.1f, 0.1f, 0.1f, 1.f }); //bottom
    window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + size + offset_right, check_bb.Min.y + size), ImVec2(check_bb.Max.x - size + offset_right, check_bb.Max.y - size), GetColorU32({ 0.18f, 0.18f, 0.18f, 1.0f }), 0.f, 0);
    window->DrawList->AddRectFilledMultiColor(ImVec2(check_bb.Min.x + size + offset_right, check_bb.Min.y + size), ImVec2(check_bb.Max.x - size + offset_right, check_bb.Max.y - size), clr3, clr3, clr4, clr4);
    window->DrawList->AddRect(ImVec2(check_bb.Min.x + size + offset_right, check_bb.Min.y + size), ImVec2(check_bb.Max.x - size + offset_right, check_bb.Max.y - size), GetColorU32({ 0.f, 0.f, 0.f, 1.0f }), 0.f, 0, 1.f);

    //checkmark
    ImU32 clr5 = ImGui::GetColorU32({ 0.53f, 0.38f, 1.f, t }); //top
    ImU32 clr6 = ImGui::GetColorU32({ 0.f, 0.f, 0.f, t }); //bottom
    window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + size + offset_right, check_bb.Min.y + size), ImVec2(check_bb.Max.x - size + offset_right, check_bb.Max.y - size), GetColorU32({ 0.53f, 0.38f, 1.f, t }), 0.f, 0);
    window->DrawList->AddRectFilledMultiColor(ImVec2(check_bb.Min.x + size + offset_right, check_bb.Min.y + size), ImVec2(check_bb.Max.x - size + offset_right, check_bb.Max.y - size), clr5, clr5, clr6, clr6);
    window->DrawList->AddRect(ImVec2(check_bb.Min.x + size + offset_right, check_bb.Min.y + size), ImVec2(check_bb.Max.x - size + offset_right, check_bb.Max.y - size), GetColorU32({ 0.f, 0.f, 0.f, t }), 0.f, 0, 1.f);




    //window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 5, check_bb.Min.y + 5), ImVec2(check_bb.Max.x - 5, check_bb.Max.y - 5), GetColorU32({ 0.25f, 0.25f, 0.25f, 1.0f }), 0.f, 0);

    //checkmark
    //window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 5, check_bb.Min.y + 5), ImVec2(check_bb.Max.x - 5, check_bb.Max.y - 5), GetColorU32({ 0.9f, 0.15f, 0.15f, t }), 0.f, 0);
...
{
    ImGui::Separator(); ImGui::Separator(); //make some space here XD
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    /*const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, 2.0f);*/
    ...
    //fuck this niggershit just make new slider function everything here is shit frfr

    ImVec2 one = ImVec2(frame_bb.Min.x + 7, frame_bb.Min.y + 10);
    ImVec2 two = ImVec2(frame_bb.Max.x - 1, frame_bb.Max.y - 1);
    ImVec2 three = ImVec2(grab_bb.Max.x, grab_bb.Max.y);

    //background
    window->DrawList->AddRectFilled(one, two, ImGui::GetColorU32({ 0.18f, 0.18f, 0.18f, 1.f }), 0.f, 15);

    //grab
    window->DrawList->AddRectFilled(one, three, GetColorU32({ 0.53f, 0.38f, 1.f, 1.f }), 0.f, 15);

    //effect (grab)
    ImU32 clr1 = ImGui::GetColorU32({ 0.53f, 0.38f, 1.f, 1.f }); //top
    ImU32 clr2 = ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }); //bottom
    window->DrawList->AddRectFilledMultiColor(one, three, clr1, clr1, clr2, clr2);

    //effect (background)
    ImU32 clr3 = ImGui::GetColorU32({ 0.18f, 0.18f, 0.18f, 1.f }); //top
    ImU32 clr4 = ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }); //bottom
    window->DrawList->AddRectFilledMultiColor(three, two, clr3, clr3, clr4, clr4);

    //value text (on the right)
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
    ImVec2 text_size_lol = CalcTextSize(value_buf);
    RenderText(ImVec2(frame_bb.Max.x + 4, 1 + frame_bb.Min.y + (text_size_lol.y / 2)), value_buf, value_buf_end);

    //label text (top of slider)
    RenderText(ImVec2(frame_bb.Min.x + 8, frame_bb.Min.y - 5), label);

    //outline
    window->DrawList->AddRect(one, two, GetColorU32({ 0.f, 0.f, 0.f, 1.f }), 0.f, 15, 1.f);
    ...
    //RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 9 - 5), ImVec2(frame_bb.Max.x - 1, frame_bb.Max.y - 3 - 5), GetColorU32({ 0.15f, 0.15f, 0.15f, 1.0f }), 4.f);
    ...
    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10 - 5), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 1 - 5), GetColorU32({ 0.9f, 0.05f + (t / 2), 0.05f + (t / 2), t + 0.8f }), 4.f);
   ...
   //background
    ...
    // Render grab
    //window->DrawList->AddRectFilled(frame_bb.Min, grab_bb.Max, GetColorU32({ 0.8f, 0.15f, 0.15f, 1.0f }), 2.0f);
